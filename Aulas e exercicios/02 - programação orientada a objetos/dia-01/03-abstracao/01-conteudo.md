...
## Recapitulando: Representação direta sem abstração
...
Essa versão **não aplica abstração**: tudo é público, e qualquer parte do código poderia alterar originador, destinatário ou mesmo valor, depois de o pix ter sido criado.

## Abstração por classes concretas
...
## Abstração por classes abstratas
...
public class PixPessoaFisica : Pix { // PixPessoaFisica está "consumindo" (ou "herdando de") Pix
...
Aqui, a classe `Pix` passou a ser um ente abstrato, que não pode ser instanciado diretamente, porém, garante que **classes consumidoras possuam um método `ImprimirComprovante()` e precisem obrigatoriamente implementar um método `Executar()`**.

## Abstração por contratos — interfaces
